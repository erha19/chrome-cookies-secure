...
const getCookies = async (uri, format, callback, profile) => {
	...
	getDerivedKey(function (err, derivedKey) {
		...
		var stmt = db.prepare("SELECT host_key, path, is_secure, expires_utc, name, value, encrypted_value, creation_utc, is_httponly, has_expires, is_persistent FROM cookies where host_key like '%" + domain + "' ORDER BY LENGTH(path) DESC, creation_utc ASC")
		var allCookies = stmt.all();
